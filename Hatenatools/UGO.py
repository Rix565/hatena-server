...
class UGO:
	...
	def Read(self, data):
		...
		if Sections >= 1:
			TableOfContents = tuple((i.split("\t") for i in data[headerlength:headerlength+self.TableLength].split(b"\n")))
		else:
			TableOfContents = []
	...
	def Pack(self):
		...
		for i in self.Items:
			if   i[0] == "unknown":
				...
				TableOfContents.append("\t".join(["0"] + list(map(str, list(i[1])))))
				...
				TableOfContents.append("\t".join(("1", num, str(labels[0]), str(labels[1]), str(labels[2]), str(labels[3]), str(labels[4]))))
			elif i[0] == "category":#2
				...
				TableOfContents.append("\t".join(("2", link, str(label), selected)))
			elif i[0] == "post":#3
				link, label = i[1:]
				...
				TableOfContents.append("\t".join(["4", link, str(trait), str(label)] + list(other)))
