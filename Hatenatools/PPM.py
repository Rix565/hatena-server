...
class PPM:
	...
	def Read(self, data, DecodeThumbnail=False, ReadFrames=True, ReadSound=False):#Load: (thumbnail, frames, sound)
		...
		self.OriginalAuthorName = data[0x14:0x2A].decode("UTF-16LE").split("\0")[0]
		self.EditorAuthorName = data[0x2A:0x40].decode("UTF-16LE").split("\0")[0]
		self.Username = data[0x40:0x56].decode("UTF-16LE").split("\0")[0]
...
def get_metadata(flipnote):
	...
	meta = {
	"Current filename":flipnote.CurrentFilename[:-3]+filetype,
	"Original filename":flipnote.OriginalFilename[:-3]+filetype,
	"Number of frames":flipnote.FrameCount,
	"Locked":flipnote.Locked,
	"Thumbnail frame index":(flipnote.ThumbnailFrameIndex+1),
	"Original author":flipnote.OriginalAuthorName,
	"Editor author":flipnote.EditorAuthorName,
	"Username":flipnote.Username,
	"Original author ID":flipnote.OriginalAuthorID,
	"Editor author ID":flipnote.EditorAuthorID,
	"Date(seconds since 1'st Jan 2000)":flipnote.Date,
	"Date":time.strftime("%H:%M %d/%m-%Y (faulty)", time.gmtime(epoch+flipnote.Date)),
	}
	if filetype == "ppm":
		meta["Frame speed"]=flipnote.Framespeed
		meta["BGM Frame speed"]=flipnote.BGMFramespeed
		meta["Looped"]=flipnote.Looped
...
if __name__ == '__main__':
	...
	if sys.argv[1] == "-t":
		print("Reading the flipnote file...", end=' ')
		...
		print("Dumping the thumbnail...", end=' ')
		...
		if len(sys.argv) < 5:
			...
			print("Dumping frame #%i..." % int(sys.argv[4]), end=' ')
		...
		print("Converting the sound files...", end=' ')
		DumpSoundFiles(flipnote,sys.argv[3],raw=(sys.argv[1]=="-S"))
		print("Done!")
		
		print("Dumping the sound effect usage...", end=' ')
		...
		print(newline.join(["\t".join([str(char) for char in line]) for line in list(meta.items())]).encode('ascii', 'ignore'))
